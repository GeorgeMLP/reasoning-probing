"""
Token-level ANOVA experiment for disentangling token vs. context effects.

This script implements a more granular ANOVA design that operates at the
token level rather than the text level. This addresses the limitation of
the text-level ANOVA where max aggregation loses token-specific information.

## Key Differences from Text-Level ANOVA

1. **Unit of analysis**: Each token position is a sample (not each text)
2. **Token factor**: Whether THIS specific token is in the reasoning token set
3. **Context factor**: Whether this text is from reasoning corpus
4. **Activation**: Feature activation at THIS token position (not max)

## Why This Works Better

Text-level ANOVA fails because:
- Reasoning text without "top-k tokens" still has many other tokens that activate
- Max aggregation washes out token-specific effects

Token-level ANOVA succeeds because:
...
```bash
python run_anova_tokenwise.py \\
    --token-analysis results/test/token_analysis.json \\
    --layer 12 \\
    --top-k-features 5 \\
    --save-dir results/test/tokenwise
```
"""
...
def main():
    ...
    # Save results
    print("\n--- Saving Results ---")
    results_path = args.save_dir / "tokenwise_anova_results.json"
